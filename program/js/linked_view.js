...
function visualization(){
  // load data, put in in lists. Make sure everything is loaded properly
  ...
  Promise.all(requests).then(function(response){
    ...
    // our list which we will use fot he colour of the map + legend
    ...
    var padding = 25;
    // link the colour values to the actual colours
    var color = d3.scaleOrdinal()
      .domain(colourScale)
      .range(["grey" ,"#ffffe5", "#f7fcb9", "#d9f0a3", "#addd8e", "#78c679", 
        "#41ab5d","#238443","#006837","#004529",  "black"]);
    var path = d3.geoPath();
    ...
    function ready(Data, Happiness, Deaths) {
      ...
      // Put all countries in right places, link life ladder color
      svg.append("g")
        ...
        .style("fill", function(d) {
          if(countriesHappiness.includes(d.properties.name)){
            var place = countriesHappiness.indexOf(d.properties.name)
            // math.ceil is used to round to the next integer, so the
            // values will be 0-1, 1-2 etc.
            HappinessMark = Math.ceil(happiness[place])
        ...
        // Only show country + life ladder score if country is in list
        ...
        // Get rid of old barchart, pie chart, slider and make 
        // new one when country clicked
        .on("click", function (d){
          ...
          function makeSlider(deathsYears){
            ...
            // Creates a svg in the horizontal direction
            var gTime = d3
          ...
          // get accurate data, put it in list and give to bar chart
          var data = [];
          HappinessNumber = 0
          if(countriesHappiness.includes(d.properties.name)){
            var place = countriesHappiness.indexOf(d.properties.name)
          ...
          // remove old piechart, make new one
